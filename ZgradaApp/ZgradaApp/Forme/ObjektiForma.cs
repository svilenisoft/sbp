    ...
    public partial class ObjektiForma : Form {
        ...
        private void izmeniObjBtn_Click(object sender, EventArgs e) {
            if (objektiListView.SelectedItems.Count == 0) {
                MessageBox.Show("Izaberite objekat koji zelite da izmenite!");
                return;
            }
            int idObjekta = Int32.Parse(objektiListView.SelectedItems[0].SubItems[0].Text);
            int rbr = Int32.Parse(objektiListView.SelectedItems[0].SubItems[1].Text);

            DodajObjekatForm forma = new DodajObjekatForm(idNivoa, tip, idObjekta, rbr, objektiListView.SelectedItems[0].SubItems[2].Text);
            forma.ShowDialog();
            fillObjektiList();
        }
        ...
        private void objektiListView_DoubleClick(object sender, EventArgs e) {
            if (tip.Equals("Stambeni nivo")) {
                new StanariForm(Int32.Parse(objektiListView.SelectedItems[0].SubItems[0].Text)).ShowDialog();
            }
        }
    }
}
