...
namespace ZgradaApp.Maperi {
    class OsobaMapiranja : ClassMap<ZgradaApp.Entiteti.Osoba> {
        public OsobaMapiranja() {
            ...
            Map(x => x.lIme, "LIME");
            Map(x => x.imeR, "IMER").Nullable();
            Map(x => x.prezime, "PREZIME");
            Map(x => x.jmbg, "JMBG").Nullable();
            Map(x => x.adresa, "ADRESA").Nullable();
            Map(x => x.brlk, "BRLK").Nullable();
            Map(x => x.mestoIzdavanja, "MESTOIZDAVANJA").Nullable();
            Map(x => x.datumRodjenja, "DATUMRODJENJA").Nullable();
